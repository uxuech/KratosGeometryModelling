...
class Gmsh3dMeshTerrain():
    ...
    def __init__(self,model,settings,TotalBoundariesNames,TotalNodesBoundaryId,Total_external_points,boundary_type,Gmsh_geometry):
    ...
    def __TopSurfaceContourDefinition(self):
        ...
        for bound in self.boundary_user_name:
            ...
            for point in self.gmsh_geometry_dictionary[bound]:
                node=gmsh.model.geo.addPoint(point[0],point[1],self.elevation)
    ...
    def __AuxiliarySkinTriangulationGmsh(self,TopNodesbyBoundary,FloorNodesbyBoundary,total_boundaries_names): 
        ...
        print(total_boundaries_names)
        for extracted_boundary_name in total_boundaries_names:
            print(extracted_boundary_name)
    ...
    def __Gmsh3dDiscreteMesh(self):
        ...
        Boundaries=self.__DictionaryForSubModelPart(AllEntitiesSurface,AllentitiesVolumes)
        print(Boundaries)
    ...
    def __DictionaryForSubModelPart(self,AllEntitiesSurface,AllentitiesVolumes):
        ...
        list_volume=[]
        BoundarybyDimension={}
        for i in AllEntitiesSurface:
            part1=gmsh.model.getPhysicalGroupsForEntity(i[0],i[1])
            part2=gmsh.model.getPhysicalName(i[0],part1[0])
            check=[part1[0],part2]
            list_surface.append(check)
        BoundarybyDimension[2]=list_surface
        for j in AllentitiesVolumes:
            part1=gmsh.model.getPhysicalGroupsForEntity(j[0],j[1])
            part2=gmsh.model.getPhysicalName(j[0],part1[0])
            
            check=[part1[0],part2]
            list_volume.append(check)
        BoundarybyDimension[3]=list_volume
